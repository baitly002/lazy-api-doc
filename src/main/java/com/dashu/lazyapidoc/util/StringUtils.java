...
public class StringUtils
{
     ...
     * @param trimLevel 1:去除前面字符 2：去除后面字符 0：前后都去除
     * @return String.
     */
    public static String trimFirstOrLastChar(String source, char element, int trimLevel) {
        if (trimLevel == 0 || trimLevel == 1){
            int beginIndex = source.indexOf(element) == 0 ? 1 : 0;
            source = source.substring(beginIndex, source.length());
        }
        if (trimLevel == 0 || trimLevel == 2){
